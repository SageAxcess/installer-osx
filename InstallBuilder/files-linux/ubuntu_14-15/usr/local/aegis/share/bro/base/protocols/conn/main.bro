...
export {
	...
	type Info: record {
		...
		## detected local_user
		local_user:        string      &log &optional;
		process:        string      &log &optional;
	};
...
event bro_init() &priority=5
	...
	Log::create_stream(Conn::Z_LOG, [$columns=LogZMQ::Info, $path="z-conn"]);
        Log::remove_default_filter(Conn::Z_LOG);
	local filter: Log::Filter = [$name="zmq", $writer=Log::WRITER_ZMQ];
	Log::add_filter(Conn::Z_LOG, filter);
	}
...
function write_log(info: Info)
	...
	local s: LogZMQ::Info = [$ptype="general", 
		$is_proto=F, $ts=info$ts, $uid=info$uid, $username=info?$local_user ? info$local_user : "", $process=info?$process ? info$process : "",
		$id=info$id, $proto=info$proto, $service=info?$service ? info$service : "", $duration=info?$duration ? info$duration : 0secs, 
		$orig_bytes=info?$orig_bytes ? info$orig_bytes : 0, $resp_bytes=info?$resp_bytes ? info$resp_bytes : 0, 
		$conn_state=info$conn_state, $local_orig=info?$local_orig ? info$local_orig : F, 
		$local_resp=info?$local_resp ? info$local_resp : F, $missed_bytes=info?$missed_bytes ? info$missed_bytes : 0, 
                $orig_pkts=info?$orig_pkts ? info$orig_pkts : 0, $resp_pkts=info?$resp_pkts ? info$resp_pkts : 0, 
		$orig_ip_bytes=info?$orig_ip_bytes ? info$orig_ip_bytes : 0, $resp_ip_bytes=info?$resp_ip_bytes ? info$resp_ip_bytes : 0
                ];

        Log::write(Conn::Z_LOG, s);
	}
...
event connection_state_remove(c: connection) &priority=-5
	{
	if( c?$local_user) c$conn$local_user = c$local_user;
	if( c?$process) c$conn$process = c$process;
