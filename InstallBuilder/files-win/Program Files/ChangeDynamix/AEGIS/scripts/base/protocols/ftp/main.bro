...
function write_log(info: Info)
	{
        Log::write(FTP::LOG, info);

#	when ( local src_host = lookup_addr(info$id$orig_h) )
#		{
		local s: LogZMQ::Info = [$ptype="ftp", $is_proto=T, $ts=info$ts, $uid=info$uid, $username=info$local_user, $process=info$local_user];

		s$ftp = info;

		Log::write(FTP::Z_LOG, s);
#		}
	}
...
event bro_init() &priority=5
	...
	Log::create_stream(FTP::Z_LOG, [$columns=LogZMQ::Info]);
        Log::remove_default_filter(FTP::Z_LOG);
	local filter: Log::Filter = [$name="zmq", $writer=Log::WRITER_ZMQ];
	Log::add_filter(FTP::Z_LOG, filter);
...
function set_ftp_session(c: connection)
	...
	if ( ! c?$ftp )
		...
		c$ftp=s;
                ...
                s$local_user=c?$local_user ? c$local_user : "";
                s$process=c?$process ? c$process : "";
